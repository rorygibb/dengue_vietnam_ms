...
source("./scripts_example/00_plot_themes.R")
source("./scripts_example/00_inla_setup_functions_r4.R")
...
dd = read.csv("./output/model_data/ModelData_Dengue_VietAll.csv") %>%
  dplyr::left_join(
    vroom::vroom("./output/model_data/ModelData_ClimateLags_VietAll.csv.gz") %>% dplyr::mutate(date = as.character(date))
    ) %>%
...
shp = shp %>% dplyr::filter(areaid %in% dd$areaid)
